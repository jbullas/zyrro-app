...
function buildGenerationContext(
...
): { context: PathReportGenerationContext; chosenCandidateId: string; preparedFor: string; identityContext: string } | null {
  ...
  return {
    ...
    // #144: cover-context-line fields — attached from data already loaded
    // here, not generated by the prompt (see PathReportContent's own comment).
    preparedFor: identityReport.cover.prepared_for,
    identityContext: identityReport.cover.identity_context,
  };
}
...
export async function GET(_req: NextRequest) {
  ...
  const { context, chosenCandidateId, preparedFor, identityContext } = built;
  ...
  if (created) {
    after(() => runGeneration(session.id, context, chosenCandidateId, preparedFor, identityContext));
