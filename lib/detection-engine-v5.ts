...
/**
 * #118 Stage 1e: standalone Detection Engine v5, evaluated in isolation per
 * docs/briefs/118-stage1e-detection-engine-v5-reused-evidence-discipline.md.
 * NOT wired into generateIdentityReport or any API route.
 * lib/detection-engine-v2/v3/v4/v6.ts all stay as-is, untouched, per each
 * stage's own Definition of Done.
 *
 * Extends v3 (not v4 — v4's populated-count instability is already a
 * settled finding, not the base here) with exactly three targeted
 * additions, none of which touch the Strong/Normal/Weak calibration
 * wording, the all-25 coverage requirement, or Architect's (or any other
 * signature's) definition:
 *
 * 1. Deduplication before extraction — targets the paraphrase-window
 *    duplication found in the linking-quality check (same underlying
 *    answer extracted twice under different wording, invisible to an
 *    exact-string reuse check).
 * 2. Equal bar for every link, regardless of order — targets the
 *    inflated-second-link pattern found in both the rating- and
 *    linking-quality checks (a link riding on an already-"spent" quote
 *    getting judged more loosely than a first link would be).
 * 3. Broader candidate scan before finalizing links — targets the
 *    systematic Originator/Pioneer-style miss (scan-breadth, not a
 *    one-off).
 *
 * Everything else is byte-identical to DETECTION_PROMPT_V3.
 */
export const DETECTION_PROMPT_V5 = `You are Zyrro's internal Signature Detection Engine (Evidence Extraction stage, signature-first design).
...
export interface EvidenceEntryV5 {
...
export interface SignatureEntryV5 {
  signature_name: string;
  evidence: EvidenceEntryV5[];
}

export interface ComputedSignatureScoreV5 {
  name: string;
  frequency: number;
  intensity: number;
  score: number;
...
/** Same formula as v2/v3/v4/v6, unchanged. */
export function computeSignatureScoresFromV5(signatures: SignatureEntryV5[]): ComputedSignatureScoreV5[] {
  const results: ComputedSignatureScoreV5[] = [];
...
export interface DetectionV5Result {
  signatures: SignatureEntryV5[];
...
/**
 * Self-contained completion call, same pattern as v3/v4/v6's run functions —
...
export async function runDetectionEngineV5(
  ...
  const response = await client.chat.completions.create({
    ...
    messages: [
      { role: 'system', content: DETECTION_PROMPT_V5 },
  ...
  let signatures: SignatureEntryV5[] = [];
  let rawParseOk = true;
  try {
