...
/**
 * #119 Stage 1: standalone Variant C ("partially-minimal"), per
 * docs/briefs/119-stage1-variant-c-partial-minimal-layer2.md. NOT wired
 * into generateIdentityReport or any API route.
 *
 * This is the live LAYER_2_PROMPT (lib/prompts/identity-report.ts) with
 * exactly three edits, each justified by a named Stage 0 finding
 * (docs/changelogs/2026-08-18.md) — everything else, including the
 * constellation_synthesis anti-overlap rule's wording, is untouched on
 * purpose per the brief's Context section (Stage 0's own data says
 * re-guessing at that rule's wording is likely to fail again, not help):
 *
 * 1. Numeric word floors/ceilings on evidence_analysis, how_you_operate,
 *    and domain_profile_summary dropped (0/30 evidence_analysis fields
 *    hit the 140-word floor across Stage 0's 6 personas; how_you_operate
 *    and domain_profile_summary also frequently missed theirs) — replaced
 *    with qualitative "write proportionate to the evidence" guidance, not
 *    removed outright, since Stage 0's fully-stripped Variant B collapsed
 *    every field category 3-6x without even qualitative guidance.
 * 2. The why_bullets worked example ("You redesigned the receiving
 *    workflow...") removed — Stage 0 traced a real fabrication directly
 *    to it (Leona's real report copied this exact sentence, describing an
 *    event absent from her actual evidence). The underlying instruction
 *    ("must name a specific fact, not a restated trait") and the
 *    word-count self-check mechanic both stay; only the example sentence
 *    that got lifted verbatim is gone.
 * 3. The Evidence Rule's "minimum 2 story anchors" softened, in both its
 *    global-rule and evidence_analysis-field-spec statements, to cite
 *    what real evidence exists without inventing a second anchor to hit a
 *    count — Stage 0 found this rule caused fabricated padding on a real
 *    1-evidence primary signature (Katalin's Pioneer entry invented a
 *    sentence about "tackling new challenges" with no source in her
 *    evidence_units).
 *
 * Reframe_teaser's word-count self-check mechanics (recap/reframe/
 * why_bullets floors), the 6-10 energisers/friction_points count, the
 * zero-secondary-signature handling instruction, and every other rule are
 * left exactly as in LAYER_2_PROMPT — Stage 0 either confirmed these are
 * load-bearing or produced no evidence either way, so nothing here is
 * touched on a guess.
 */
export const VARIANT_C_LAYER2_PROMPT = `You are Zyrro's Layer 2 Report Engine.
...
### how_you_operate
Five fields. Each a full paragraph, several sentences — proportionate depth, not a one-line summary. Each grounded in the user's actual patterns.
...
- 5 how_you_operate fields, each a full paragraph, proportionate to the evidence available
- 6–10 energisers
...
/**
 * #120: standalone Variant C′ ("C prime"), per
 * docs/briefs/120-variant-c-prime-how-you-operate-floor.md. NOT wired into
 * generateIdentityReport or any API route.
 *
 * Ships Variant C (`VARIANT_C_LAYER2_PROMPT` above) — the fabrication fix
 ...
 * anchor") — with one targeted modification: `how_you_operate`'s field
 * spec keeps the live `LAYER_2_PROMPT`'s numeric minimum/target structure
 * ("Each minimum 120 words, target 150 words") instead of Variant C's
 * qualitative "proportionate depth" swap. `evidence_analysis` and
 * `domain_profile_summary` keep Variant C's qualitative wording unchanged
 * — #119 Stage 1 found the numeric-floor removal cost `how_you_operate`
 * real length (41-65 words/field vs. live's 68-107) without a comparable
 * regression in `evidence_analysis`, matching the #112 Stage 2 Round 1
 * precedent that dropping "minimum X, target Y" structure shrinks output
 * independent of the stated number. This isolates that one variable:
 * does restoring just the numeric structure (not the underlying content
 * spec, which is otherwise byte-identical to Variant C's) recover some of
 * that length back.
 *
 * Derived from `VARIANT_C_LAYER2_PROMPT` via a scripted text transform
 * (`scripts/pilot-120-gen-variant-cprime.mts`, deleted after use), not
 * hand-retyped — confirmed programmatically that exactly two spots differ
 * from Variant C: the `### how_you_operate` opening sentence, and the
 * matching `## STRUCTURAL CONSTRAINTS` summary line (updated to the same
 * numeric framing for internal consistency, since leaving it as Variant
 * C's "proportionate to the evidence" wording while the field spec above
 * it says "minimum 120, target 150" would contradict itself). Every other
 * character, including the Evidence Rule softening and the removed
 * why_bullets example, is unchanged from Variant C.
 */
export const VARIANT_C_PRIME_LAYER2_PROMPT = `You are Zyrro's Layer 2 Report Engine.
...
/**
 * #119 Stage 4: standalone Variant F, per
 * docs/briefs/119-stage4-variant-f-how-you-operate-retry.md. NOT wired into
 * generateIdentityReport or any API route.
 *
 * Second attempt at fixing how_you_operate's length regression (Stage 1),
 * informed by Stage 3's finding that the cause is content-specific, not
 * generic prompt length. Two targeted fixes to Variant D's wording:
 * (1) removed the "not three sentences" hedge, which plausibly read as
 * permission to compress rather than a floor, replaced with an explicit
 * "each move needs its own full sentence" requirement and an explicit ban
 * on compressing all three moves into one or two sentences; (2) strengthened
 * the "situation" move to require a concrete, nameable circumstance already
 * evident in the person's material, not a restated version of the pattern
 * itself — matching evidence_analysis's own Evidence Rule bar, which Stage 2
 * showed Variant D's version never actually met. No numeric floor, no
 * worked example — both already-confirmed dead weight/fabrication vectors
 * from Stage 0, left out on purpose. Everything else byte-identical to
 * VARIANT_C_LAYER2_PROMPT (verified programmatically at generation time,
 * see scripts/pilot-119-gen-variant-f.mts, deleted after use).
 */
export const VARIANT_F_LAYER2_PROMPT = `You are Zyrro's Layer 2 Report Engine.
...
### how_you_operate
Five fields. Each field must read as a full paragraph — several complete sentences, not a compressed one- or two-sentence summary. Each follows three moves, and each move needs its own full sentence, not a clause: (1) state the pattern, (2) ground it in one specific real situation, habit, or choice already evident in this person's material — a concrete circumstance you could point to, not a restated version of the pattern itself, (3) name what that pattern requires or costs in practice. Compressing these three moves into one or two sentences total is not acceptable, even if all three ideas are technically present — each needs room to be a real sentence. Each grounded in the user's actual patterns.
...
/**
 * #119 Stage 3: standalone Variant E, per
 * docs/briefs/119-stage3-variant-e-length-diagnostic.md. NOT wired into
 * generateIdentityReport or any API route.
 *
 * Diagnostic control for Stage 2's open question: did Variant D's
 * how_you_operate fix backfire because of *what* it added, or because
 * adding *any* text of that length anywhere in the prompt suppresses
 * generation elsewhere, independent of content? Variant E = Variant C with
 * ~286 characters of inert padding inserted into the WRITING
 * PRINCIPLE section (restates the existing "lived, not labelled"
 * distinction in different words, extended to apply report-wide — no new
 * instruction, no new content requirement, nothing actionable that wasn't
 * already true). how_you_operate's field spec is byte-identical to Variant
 * C's (verified programmatically at generation time, see
 * scripts/pilot-119-gen-variant-e.mts, deleted after use) — if
 * how_you_operate still contracts under E despite never being touched, the
 * cause is generic (prompt length/density), not Variant D's specific
 * wording.
 */
export const VARIANT_E_LAYER2_PROMPT = `You are Zyrro's Layer 2 Report Engine.
...
Identity must feel lived. Not labelled. This distinction — lived, not labelled — is not limited to primary signature writing. It applies to every section of this report where identity is described, not just the ones written about it most directly, since a report that only names traits is not the same as one that shows them.
...
/**
 * #119 Stage 2: standalone Variant D, per
 * docs/briefs/119-stage2-variant-d-how-you-operate-fix.md. NOT wired into
 * generateIdentityReport or any API route.
 *
 * Variant C (Stage 1) with exactly one further change: how_you_operate's
 * field spec. Stage 1 found dropping the numeric word floor over-contracted
 * how_you_operate ~35-45% (unlike evidence_analysis, which held its length
 * via the retained Pattern→Evidence→Meaning scaffold — how_you_operate's
 * five fields never had an equivalent structure, only a one-line content
 * description each). Fix: give how_you_operate the same kind of qualitative
 * three-part scaffold (pattern → specific real situation → what it costs/
 * requires) that already works for evidence_analysis — not a restored
 * numeric floor, which Stage 0 already showed is dead weight regardless.
 *
 * Everything else is byte-for-byte identical to VARIANT_C_LAYER2_PROMPT,
 * per the brief's explicit scope: no numeric floors added back anywhere,
 * no other field touched.
 */
export const VARIANT_D_LAYER2_PROMPT = `You are Zyrro's Layer 2 Report Engine.
...
### how_you_operate
Five fields. Each field follows three moves: state the pattern, ground it in one specific real way it shows up for this person (a situation, habit, or choice from their evidence — not a restated trait), then note what that pattern requires or costs in practice. Three moves, not three sentences — write however many sentences each move needs, but all three must be present. Each grounded in the user's actual patterns.
