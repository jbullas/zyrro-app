...
/**
 * #118 Stage 1b: standalone Detection Engine v3, evaluated in isolation per
 * docs/briefs/118-stage1b-signature-first-detection-engine-v3.md. NOT wired
 * into generateIdentityReport or any API route — the live pipeline is
 * unchanged. lib/detection-engine-v2.ts (Stage 1) also stays as-is,
 * untouched, per Stage 1's own Definition of Done.
 *
 * Inverse structure from v2: v2 freeform-extracted undefined evidence
 * chunks first and matched each to whichever signature(s) it supported
 * afterward (bottom-up). v3 asks about all 25 signatures explicitly, in
 * one call, and requires an answer (evidence-or-none) for each
 * (signature-first / top-down) — testing whether a narrower, defined
 * question asked 25 times is more stable than one open-ended extraction
 * question asked once, and whether it removes the implicit
 * decide-to-extract pre-filter suspected of suppressing Weak ratings in
 * v2 (see Stage 1's 2026-08-11 changelog entry).
 */
export const DETECTION_PROMPT_V3 = `You are Zyrro's internal Signature Detection Engine (Evidence Extraction stage, signature-first design).
...
export interface EvidenceEntryV3 {
...
export interface SignatureEntryV3 {
  signature_name: string;
  evidence: EvidenceEntryV3[];
}

export interface ComputedSignatureScoreV3 {
  name: string;
  frequency: number;
  intensity: number;
  score: number;
...
/**
 * Same formula as v2's computeSignatureScoresFromEvidenceUnits, unchanged
 * per the brief ("Computed scoring: Unchanged from Stage 1"), just applied
 * to v3's already-per-signature evidence arrays instead of aggregating
 * from cross-signature links.
 */
export function computeSignatureScoresFromV3(signatures: SignatureEntryV3[]): ComputedSignatureScoreV3[] {
  const results: ComputedSignatureScoreV3[] = [];
...
export interface DetectionV3Result {
  signatures: SignatureEntryV3[];
...
/**
 * Standalone completion call, deliberately not routed through lib/llm.ts's
 * shared getChatCompletion — that function is used by the live pipeline
 * and doesn't surface token usage/timing, and this stage's check 6 (real
 * cost/latency numbers) needs both. Keeping this self-contained here means
 * the live pipeline's shared helper isn't touched at all for a standalone
 * experiment.
 */
export async function runDetectionEngineV3(
  answers: Array<{ question_number: number; question_text: string; answer_text: string }>
): Promise<DetectionV3Result> {
  ...
  const response = await client.chat.completions.create({
    ...
    messages: [
      { role: 'system', content: DETECTION_PROMPT_V3 },
  ...
  let signatures: SignatureEntryV3[] = [];
  let rawParseOk = true;
  try {
