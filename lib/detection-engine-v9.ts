...
/**
 * #118 Stage 1i: standalone Detection Engine v9, evaluated in isolation per
 * docs/briefs/118-stage1i-detection-engine-v9-pairwise-comparison.md. NOT
 * wired into generateIdentityReport or any API route.
 * lib/detection-engine-v2/v3/v4/v5/v6/v7/v8.ts all stay as-is, untouched,
 * per each stage's own Definition of Done.
 *
 * Extends v5 (v3 base + Fix 1 dedup + Fix 3 broader scan). Fix 2's
 * "equal bar, judge independently" instruction is removed entirely and
 * replaced with a direct pairwise-comparison instruction — three separate
 * call-architecture attempts (v5 self-applied instruction, v7 forced
 * two-phase single-call review, v8 a genuinely separate cold second call)
 * all converged on the same failure on the same two motivating cases
 * (Matteo's Architect/Contextualiser Q5, Originator/Pioneer Q3), and v8's
 * zero-change result specifically ruled out shared-context/
 * self-defensiveness as the mechanism. This stage tests a different
 * hypothesis instead: every prior attempt asked the model to judge each
 * link "independently," which may be exactly the framing that prevents the
 * one comparison (two links, read side by side) that would expose a
 * redundant pair. This is a single-call test, not a fourth call-splitting
 * variant.
 */
export const DETECTION_PROMPT_V9 = `You are Zyrro's internal Signature Detection Engine (Evidence Extraction stage, signature-first design).
...
export interface EvidenceEntryV9 {
...
export interface SignatureEntryV9 {
  signature_name: string;
  evidence: EvidenceEntryV9[];
}

export interface ComputedSignatureScoreV9 {
  name: string;
  frequency: number;
  intensity: number;
  score: number;
...
/** Same formula as v2 through v8, unchanged. */
export function computeSignatureScoresFromV9(signatures: SignatureEntryV9[]): ComputedSignatureScoreV9[] {
  const results: ComputedSignatureScoreV9[] = [];
...
export interface DetectionV9Result {
  signatures: SignatureEntryV9[];
...
/**
 * Self-contained completion call, same pattern as every prior standalone
 * engine version — its own OpenAI client instance, not routed through
 * lib/llm.ts.
 */
export async function runDetectionEngineV9(
  answers: Array<{ question_number: number; question_text: string; answer_text: string }>
): Promise<DetectionV9Result> {
  ...
  const response = await client.chat.completions.create({
    ...
    messages: [
      { role: 'system', content: DETECTION_PROMPT_V9 },
  ...
  let signatures: SignatureEntryV9[] = [];
  let rawParseOk = true;
  try {
